  ...
  switch(req.method) {
    case 'GET':
      if (pathname == '/') {
        ...
        if (filePath.indexOf(__dirname) != 0 || pathname.slice(1).includes('/') || pathname.includes('..')) {
          breakResponse(res, 400, "Nested paths and stepping out of the folder are not allowed")
          return
        }
    ...
    case 'POST':      
      if (!filePath || !filePath.length) {        
        breakResponse(res, 400, "File is not specified")
        return
      }
      //stepping out of allowed folder
